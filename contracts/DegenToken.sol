...
contract DegenToken is ERC20 {
    ...
    mapping (address => bool)  redeemedPlayer;

    mapping (address => bool)  redeemJersey;

    mapping (address => bool)  redeemFootballPitch;
    ...
    error InvalidItem();
    error RedeemPlayerFirst();
    error RedeemJerseyFirst();
    ...
    function viewMarketplace() external pure returns (string memory) {
        return "Marketplace: 1. Player - 15DGN, 2. Jersey - 25DGN, 3. Football - 50DGN ";
    }
    ...
    function redeemMarketPlaceItem(uint8 _itemToredeem) external returns (bool) {
        ...
        if (_itemToredeem == 1) {
            ...
            redeemedPlayer[msg.sender] = true;
            
            return true;
        
        } else if (_itemToredeem == 2) {
            
            if (!redeemedPlayer[msg.sender]) revert RedeemPlayerFirst();
            ...
            redeemJersey[msg.sender] = true;
            
            return true;

        } else if (_itemToredeem == 3) {

            if (!redeemJersey[msg.sender]) revert RedeemJerseyFirst();
            ...
            redeemFootballPitch[msg.sender] = true;
